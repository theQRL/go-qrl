...
// chainContext constructs the context reader which is used by the qrvm for reading
// the necessary chain context.
func (api *API) chainContext(ctx context.Context) core.ChainContext {
	return qrlapi.NewChainContext(ctx, api.backend)
}
...
type TraceCallConfig struct {
	...
	BlockOverrides *qrlapi.BlockOverrides
}
...
// TraceChain returns the structured logs created during the execution of QRVM
...
// QRVM and returns them as a JSON object.
...
// TraceBlock returns the structured logs created during the execution of QRVM
...
// QRVM against a block pulled from the pool of bad ones and returns them as a JSON
// object.
...
// execution of QRVM to the local file system and returns a list of files
// to the caller.
...
func (api *API) IntermediateRoots(ctx context.Context, hash common.Hash, config *TraceConfig) ([]common.Hash, error) {
	...
	for i, tx := range block.Transactions() {
		...
		var (
			...
			vmenv     = vm.NewQRVM(vmctx, txContext, statedb, chainConfig, vm.Config{})
		)
...
// execution of QRVM against a block pulled from the pool of bad ones to the
...
txloop:
	for i, tx := range txs {
		...
		msg, _ := core.TransactionToMessage(tx, signer, block.BaseFee())
		statedb.SetTxContext(tx.Hash(), i)
		vmenv := vm.NewQRVM(blockCtx, core.NewQRVMTxContext(msg), statedb, api.backend.ChainConfig(), vm.Config{})
		if _, err := core.ApplyMessage(vmenv, msg, new(core.GasPool).AddGas(msg.GasLimit)); err != nil {
			failed = err
			break txloop
		}
...
func (api *API) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	for i, tx := range block.Transactions() {
		...
		vmenv := vm.NewQRVM(vmctx, txContext, statedb, chainConfig, vmConf)
		statedb.SetTxContext(tx.Hash(), i)
...
func (api *API) traceTx(ctx context.Context, message *core.Message, txctx *Context, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig) (any, error) {
	var (
		...
		txContext = core.NewQRVMTxContext(message)
	)
	if config == nil {
		config = &TraceConfig{}
	}
	...
	vmenv := vm.NewQRVM(vmctx, txContext, statedb, api.backend.ChainConfig(), vm.Config{Tracer: tracer, NoBaseFee: true})
	...
	go func() {
		...
		if errors.Is(deadlineCtx.Err(), context.DeadlineExceeded) {
			...
			// Stop qrvm execution. Note cancellation is not necessarily immediate.
			vmenv.Cancel()
		}
	}()
	defer cancel()

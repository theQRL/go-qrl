...
func ReadSkeletonSyncStatus(db qrldb.KeyValueReader) []byte {
	data, _ := db.Get(skeletonSyncStatusKey)
	return data
}

// WriteSkeletonSyncStatus stores the serialized sync status to save at shutdown.
func WriteSkeletonSyncStatus(db qrldb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSkeletonSyncStatus(db qrldb.KeyValueWriter) {
...
func ReadSkeletonHeader(db qrldb.KeyValueReader, number uint64) *types.Header {
...
func WriteSkeletonHeader(db qrldb.KeyValueWriter, header *types.Header) {
...
func DeleteSkeletonHeader(db qrldb.KeyValueWriter, number uint64) {

...
func ReadDatabaseVersion(db qrldb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db qrldb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db qrldb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db qrldb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func ReadGenesisStateSpec(db qrldb.KeyValueReader, blockhash common.Hash) []byte {
	data, _ := db.Get(genesisStateSpecKey(blockhash))
	return data
}

// WriteGenesisStateSpec writes the genesis state specification into the disk.
func WriteGenesisStateSpec(db qrldb.KeyValueWriter, blockhash common.Hash, data []byte) {
...
func PushUncleanShutdownMarker(db qrldb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db qrldb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
...
func UpdateUncleanShutdownMarker(db qrldb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)

...
type (
	executionFunc func(pc *uint64, interpreter *QRVMInterpreter, callContext *ScopeContext) ([]byte, error)
	gasFunc       func(*QRVM, *Contract, *Stack, *Memory, uint64) (uint64, error) // last parameter is the requested memory size as a uint64
...
func newShanghaiInstructionSet() JumpTable {
	tbl := JumpTable{
		...
		SLOAD: {
			execute:     opSload,
			constantGas: 0,
			dynamicGas:  gasSLoadEIP2929,
			minStack:    minStack(1, 1),
			maxStack:    maxStack(1, 1),
		},
		SSTORE: {
			execute:    opSstore,
			dynamicGas: gasSStoreEIP3529,
			minStack:   minStack(2, 0),
			maxStack:   maxStack(2, 0),
		},
		JUMP: {
		...
		STATICCALL: {
			...
			constantGas: params.WarmStorageReadCostEIP2929,
			dynamicGas:  gasStaticCallEIP2929,
			minStack:    minStack(6, 1),
			maxStack:    maxStack(6, 1),

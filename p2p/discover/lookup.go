...
func newLookup(ctx context.Context, tab *Table, target qnode.ID, q queryFunc) *lookup {
	it := &lookup{
		tab:       tab,
		queryfunc: q,
...
func (it *lookupIterator) Node() *qnode.Node {
	if len(it.buffer) == 0 {
		return nil
	}
